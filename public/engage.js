...
2. SCHEDULED DEMO NOT DELIVERED (demo/solution stage but no demo occurred):
  ...
  async function detectUnknownParticipants(notes, prospect, contactTitle, rep) {
    try {
      ...
      // Reps who have appeared on prior calls with this account — generic-labeled speakers
      // matching their behavior should resolve to these names rather than being flagged unknown.
      const priorAccountReps = prospect
        ? [...new Set(
            loadHistory()
              .filter(h => (h.prospect || '').trim().toLowerCase() === prospect.trim().toLowerCase())
              .flatMap(h => _histParseRepScores(h.rep_scores).map(r => r.name).filter(Boolean))
          )]
        : [];
      const priorAccountRepsCtx = priorAccountReps.length
        ? `Sales reps who have appeared on PRIOR calls with ${prospect}: ${priorAccountReps.join(', ')}. They are likely present on this call too, even if this transcript only labels them generically (e.g. "Speaker 2") instead of by name — if a generic-labeled speaker's behavior matches one of these reps (selling/presenting language, technical answers, etc.) and that rep's real name isn't otherwise mentioned, resolve them to that rep's name and classify as sales_team rather than unknown.`
        : '';
